...
export function renderSidebar(permsHex, currentPath, isDeptApprover = false, companyName = 'Ferry Portal', siteLogo = '', roleName = null) {
    ...
    if (can('user_management.access') || can('schedule_management.access') || can('approval_workflow.configure_hierarchy')
        || can('booking.view_all') || can('reports.view_admin') || can('audit_logs.access') || can('branding.access') || can('settings.access') || can('settings.manage_email') || can('booking.bulk_transfer_passengers') || can('booking.view_resort_capacity') || can('booking.manage_supplier_reservations') || isAdmin) {
        ...
        if (can('booking.view_resort_capacity')) links.push(navLink('/admin/resort_capacity', 'bi-pie-chart', 'Resort Capacity Allocator', currentPath));
        if (can('booking.manage_supplier_reservations')) links.push(navLink('/admin/supplier_reservations', 'bi-person-badge', 'Supplier Visit Reservations', currentPath));
        ...
        if (can('audit_logs.view_activity') || can('audit_logs.view_permission_changes') || can('audit_logs.view_hr_manual_bookings') || can('audit_logs.view_seat_reservations') || can('audit_logs.view_email_log') || can('audit_logs.view_hod_seat_assignments') || can('audit_logs.view_ferry_transfers') || can('audit_logs.view_resort_capacity') || can('audit_logs.view_ferry_services')) {
