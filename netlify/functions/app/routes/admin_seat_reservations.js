...
async function reservationsPageBody({ statusFilter, resortFilter, csrfToken }) {
    let query = db()
        ...
        .select(
            'reservation_id, schedule_id, reservation_type, seats, start_date, end_date, weekdays, reason, status, resort_id, ' +
...
(function () {
    ...
    function sync() {
        ...
        departmentField.style.display = typeSelect.value === 'department' ? '' : 'none';
    }
...
<div class="card shadow-sm"><div class="table-responsive"><table class="table table-hover mb-0 align-middle">
    <thead><tr><th>Schedule</th><th>Resort</th><th>Type</th><th>Employee / Department</th><th>Seats</th><th>Period</th><th>Status</th><th>Actions</th></tr></thead>
    <tbody>${raw(rowsHtml || '<tr><td colspan="8" class="text-center text-muted py-4">No seat reservations found.</td></tr>')}</tbody>
</table></div></div>
${raw(createModalHtml)}
${raw(editModalsHtml)}`;
}
...
export function registerAdminSeatReservationsRoutes(router) {
    ...
    router.post('/admin/seat_reservations', async (request) => {
        ...
        if (form.action === 'create') {
            ...
            const departmentId = reservationType === 'department' && form.department_id ? Number(form.department_id) : null;
            ...
            const inserted = unwrap(
                await db()
                    ...
                    .insert({
                        ...
                        employee_user_id: employeeUserId, department_id: departmentId, seats,
        ...
        if (form.action === 'edit') {
            ...
            unwrap(await db().from('seat_reservations').update({ seats, start_date: startDate, end_date: endDate, weekdays, reason }).eq('reservation_id', reservationId));
        ...
        if (form.action === 'release' || form.action === 'cancel') {
            ...
            const rows = unwrap(
                await db()
                    .from('seat_reservations')
                    .select(
                        'reservation_id, schedule_id, resort_id, reservation_type, seats, start_date, end_date, status, ' +
                            'employee:users!seat_reservations_employee_user_id_fkey(full_name), department:departments(department_name), ferry_schedule(ferry_routes(direction))'
                    )
                    .eq('reservation_id', reservationId)
                    .limit(1)
